...
var daemon = function(config){
   ...
   */
    ...
    /**
     * @method build
     * Build the service executable for the script.
     * @param {String} [dir=root of script]
     * The directory where the process files will be saved. Defaults to #script path.
     * @param {Function} [callback]
     * The callback to fire when the build completes.
     */
    /**
     * @event build
     * Fired when the build process is complete.
     */
     /**
      * @event alreadybuilt
      * Fired if the executable targets already exist.
      */
    /**
     * @event error
    ...
    build: {
      ...
      value: function(dir,callback){
        ...
        if (this.exists){
          ...
          this.log.warn('The executable cannot be built because it already exists.');
          this.emit('alreadybuilt');
          return;
        }
    ...
    /**
     * @method clean
     * Clean the build service executables.
    /**
     * @event clean
     * Fired when the clean is complete.
     */
    /**
     * @event alreadycleaned
     * Fired if the executable targets do not exist.
     */
    clean: {
      enumerable: true,
      writable: false,
      value: function(){
        ...
        if (!this.exists){
          console.log('Clean was skipped because process does not exist or could not be found.');
          this.emit('alreadycleaned');
          return;
        }
        ...
        this.once('stop',cleaner);
        this.once('alreadystopped',cleaner);
        this.stop();
      }
    },
     ...
     * Determine whether the service has been installed.
     */
    installed: {
      enumerable: true,
      get: function(){
